  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid h-[calc(100vh-260px)] gap-6 xl:grid-cols-[0.95fr_1.6fr]">
        <div className="flex flex-col rounded-[1.6rem] border border-[#C8FF00]/10 bg-[#0E1116]/90">
          <div className="flex items-center justify-between border-b border-[#C8FF00]/10 p-4">
            <h2 className="font-semibold text-white">Conversations</h2>
            <button onClick={handleNewConversation} className="rounded-full bg-[#C8FF00]/10 p-2 text-[#C8FF00] transition hover:bg-[#C8FF00]/20">
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.length === 0 ? (
              <div className="p-4 text-center text-[#F8F9FA]/60">
                <MessageCircle className="mx-auto mb-2 h-8 w-8 text-[#C8FF00]/30" />
                <p className="text-sm">No conversations yet</p>
              </div>
            ) : (
              conversations.map((conv) => (
                <div key={conv.id} onClick={() => setSelectedConversation(conv)} className={`cursor-pointer border-b border-[#C8FF00]/5 p-4 transition ${selectedConversation?.id === conv.id ? 'bg-[#C8FF00]/10 border-l-2 border-l-[#C8FF00]' : 'hover:bg-[#C8FF00]/5'}`}>
        ...
        <div className="flex flex-col rounded-[1.6rem] border border-[#C8FF00]/10 bg-[#0E1116]/90">
          {selectedConversation ? (
            <>
              <div className="border-b border-[#C8FF00]/10 p-4">
                  ...
                  messages.map((msg) => (
                    <div key={msg.id} className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-[#C8FF00]">{msg.sender?.name || 'Unknown'}</span>
                        <span className="text-xs text-[#F8F9FA]/50">{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                      <div className="rounded-[1.1rem] border border-[#C8FF00]/20 bg-[#C8FF00]/10 p-3 text-white break-words">{msg.content}</div>
                    </div>
                  ))
                )}
              </div>
              <div className="flex gap-2 border-t border-[#C8FF00]/10 p-4">
                <input type="text" placeholder="Type a message..." value={messageInput} onChange={(e) => setMessageInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} className="flex-1 rounded-full border border-[#C8FF00]/10 bg-[#07070A] px-4 py-2 text-white outline-none placeholder:text-[#F8F9FA]/40" />
                <button onClick={handleSendMessage} className="rounded-full bg-[#C8FF00] px-4 py-2 font-semibold text-[#07070A] transition hover:bg-[#C8FF00]/90">
            ...
            <div className="flex h-full items-center justify-center text-center text-[#F8F9FA]/60">
              <div>
                <MessageCircle className="mx-auto mb-4 h-12 w-12 text-[#C8FF00]/30" />
