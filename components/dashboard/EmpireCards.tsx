...
const panelBase =
  'rounded-[1.75rem] border border-[#D4AF37]/10 bg-[#0E1116]/90 p-5 shadow-[0_0_0_1px_rgba(212,175,55,0.06),0_22px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl';
...
export function MetricCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="text-sm text-[#D4AF37]/70">{title}</p>
    ...
    <div className={`${panelBase} space-y-4`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {subtitle ? <p className="mt-1 text-sm text-[#D4AF37]/70">{subtitle}</p> : null}
        </div>
        {actionLabel && actionHref ? (
          <a href={actionHref} className="inline-flex items-center gap-2 text-sm font-medium text-[#D4AF37] hover:text-[#F8F9FA]">
            {actionLabel}
...
export function ModuleCard({
  ...
  return (
    <a
      href={href}
      className="group rounded-[1.35rem] border border-[#D4AF37]/10 bg-[#11151E] p-4 transition hover:-translate-y-1 hover:border-[#D4AF37]/40 hover:bg-[#161C27]"
    >
      <div className="flex items-start justify-between gap-3">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#D4AF37]/70">{role}</p>
          </div>
        </div>
        <span className="rounded-full border border-[#10B981]/20 bg-[#10B981]/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-[#10B981]">
          {status}
