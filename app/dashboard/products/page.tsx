        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[1.6rem] border border-[#C8FF00]/20 bg-[#0E1116] p-6">
            ...
            <div className="space-y-4">
              ...
              <select value={newProduct.type} onChange={(e) => setNewProduct({ ...newProduct, type: e.target.value as any })} className="w-full rounded-[1.1rem] border border-[#C8FF00]/20 bg-[#07070A] px-4 py-2.5 text-white outline-none focus:border-[#C8FF00]">
      ...
      <SectionPanel title="Product catalog" subtitle="Move from concept to commerce with a clear, premium set of offers">
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {products.map((product) => (
            <div key={product.id} className="rounded-[1.35rem] border border-[#C8FF00]/10 bg-[#11151E] p-6 transition hover:border-[#C8FF00]/30">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full bg-[#C8FF00]/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-[#C8FF00]">{product.status}</span>
              </div>
              {product.description ? <p className="mt-4 text-sm text-[#F8F9FA]/60">{product.description}</p> : null}
              <div className="mt-5 rounded-[1.1rem] border border-[#C8FF00]/10 bg-[#07070A] p-3">
                <p className="text-xs uppercase tracking-[0.24em] text-[#C8FF00]/60">Price</p>
        ...
        {products.length === 0 ? (
          <div className="rounded-[1.35rem] border border-[#C8FF00]/10 bg-[#11151E] p-12 text-center">
            <Package className="mx-auto mb-4 h-12 w-12 text-[#C8FF00]/30" />
            <p className="mb-4 text-[#F8F9FA]/60">No products yet</p>
            <button onClick={() => setShowCreateModal(true)} className="rounded-full bg-[#C8FF00] px-6 py-2 font-semibold text-[#07070A] transition hover:bg-[#C8FF00]/90">Add your first product</button>
          </div>
        ) : null}
      </SectionPanel>
    </div>
