...
export default function InvoiceDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!invoice) {
    return (
      <div className="text-center p-12 text-[#D4AF37]/70">Invoice not found.</div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-[#D4AF37] px-5 py-3 text-sm font-semibold text-[#07070A] hover:bg-[#D4AF37]/90 transition disabled:opacity-60"
          >
      ...
      <div className="grid gap-6 xl:grid-cols-[2fr_1fr]">
        <section id="invoice-preview" className="space-y-6 rounded-3xl border border-[#D4AF37]/10 bg-[#0E1116] p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.24em] text-[#D4AF37]/70">Invoice</p>
              <h1 className="text-3xl font-bold text-white">{invoice.invoice_number}</h1>
              <p className="text-[#D4AF37]/70 mt-2">Created on {new Date(invoice.created_at).toLocaleDateString()}</p>
            </div>
            <span className="inline-flex items-center gap-2 rounded-full bg-[#11151E] px-4 py-2 text-sm text-[#D4AF37]">
        ...
        <aside className="space-y-6 rounded-3xl border border-[#D4AF37]/10 bg-[#0E1116] p-6">
          <div className="space-y-3">
            <p className="text-sm uppercase tracking-[0.24em] text-[#D4AF37]/70">Payment status</p>
            <p className="text-3xl font-semibold text-white">{invoice.status?.toUpperCase()}</p>
          </div>
          <div className="rounded-3xl bg-[#11151E] p-4">
            <p className="text-sm text-[#D4AF37]/60">Next step</p>
